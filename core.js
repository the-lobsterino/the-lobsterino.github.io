...
class Core {
    ...
    createCorona() {
        ...
        const coronaFrag = `
            ...
            uniform vec3 uCorePosition;  // Core position in screen space
            uniform float uCoreRadius;   // Apparent radius
            ...
            void main() {
                ...
                vec2 center = vec2(0.5) + uCorePosition.xy * 0.5;
                ...
                // Core apparent radius (normalized)
                float coreR = uCoreRadius;
                
                // Animated rays
                float t = uTime * 0.05;
                
                // Multiple ray layers with different speeds
                float rays = 0.0;
                ...
                // Radial falloff - smooth transition at core edge
                float innerFade = smoothstep(coreR * 0.8, coreR * 1.05, dist);
                float outerFade = 1.0 - smoothstep(coreR, coreR + 0.5, dist);
                float falloff = innerFade * outerFade;
                
                // Ray intensity modulation
                float rayIntensity = rays * 0.6 + 0.4;
                
                // Final intensity
                float intensity = falloff * rayIntensity;
                
                // Colors - warm orange, no green tint
                vec3 colorInner = vec3(1.0, 0.7, 0.35);
                vec3 colorOuter = vec3(1.0, 0.35, 0.08);
                vec3 color = mix(colorOuter, colorInner, outerFade);
                
                // Skip if too weak (performance + cleaner edges)
                if (intensity < 0.01) discard;
                
                // Apply intensity to color, clamp to prevent oversaturation
        ...
        // Fullscreen quad
        const coronaGeom = new THREE.PlaneGeometry(2, 2);
        ...
        this.corona.renderOrder = -1;  // Render before core
